...
/*here for pwm config*/
/* AO_PWM_MISC_REG_AB(PWM_B_CLK_DIV) [22:16] 0-0x7f */
static unsigned int pwm_prescale = 0x03;
/* AO_PWM_PWM_B(PWM_B_DUTY_CYCLE) [31:15]H,[15-0]L   //915KHz */
static unsigned int pwm_entire_cycle = 0xFFFF;
...
static unsigned int blink_status;
/* led mode */
unsigned int led_mode_s = 0;
/* led breath timeout 20ms*/
static unsigned int led_breath_timeout;
...
static void g_timer_handle(unsigned long arg)
{
	...
	if (led_breath_timeout > BREATH_TIMEOUT_COUNT) {
		...
		pr_info("led breath 15s timeout.\n");
		return;
	}
...
static void g_timer_init(unsigned int timer_expires_ms)
{
	...
	g_timer.expires = jiffies + msecs_to_jiffies(timer_expires_ms);
}

static void g_timer_start(unsigned int timer_expires_ms)
{
	/* HZ / 1000 * timer_expires_ms); */
	mod_timer(&g_timer, jiffies + msecs_to_jiffies(timer_expires_ms));
}
...
static void g_timer_del(void)
{
	...
	if (ret)
		...
		pr_info("The timer init but not use...\n");
}
...
static void leds_pwm_init(unsigned int prescale, unsigned int cycle)
{
	...
	/* clear PWM_B_CLK_DIV(22-16),reserve PWM_A*/
	val = readl(pdata->reg_base + PWM_MISC_REG_AB) & 0xFF80FFFF;
	val |=
	    PWM_B_CLK_DRV(prescale) | PWM_B_EN | PWM_B_CLK_EN |
	    PWM_B_CLK_SEL(0);
	pr_info("%s writel:0x%x\n", __func__, val);
	writel(val, pdata->reg_base + PWM_MISC_REG_AB);

	writel(cycle, pdata->reg_base + PWM_PWM_B);
}
...
/*breathled class*/
static ssize_t class_set_led_mode(struct class *cla,
...
static ssize_t class_get_led_mode(struct class *cla,
...
{
	pr_info("enter aml_breathled_early_suspend\n");
	led_control(ALWAYS_OFF);
}

static void aml_breathled_late_resume(struct early_suspend *dev)
{
	pr_info("enter aml_breathled_late_resume\n");
	pr_info("dgt, aml_breathled_late_resume breath led\n");
	led_control(ALWAYS_ON);
}
#endif
/*suspend&resume*/
static int aml_breathled_suspend(struct platform_device *pdev,
				 pm_message_t state)
{
	pr_info("enter aml_breathled_suspend\n");
	return 0;
}

static int aml_breathled_resume(struct platform_device *pdev)
{
	pr_info("enter aml_breathled_resume\n");
	if (get_resume_method() == REMOTE_WAKEUP
#ifdef AUTOSUSPEND_TEST
	    || get_resume_method() == AUTO_WAKEUP
#endif
	    ) {
		pr_info("breathled_resume:led on\n");
		led_control(ALWAYS_ON);
	}
	return 0;
}
...
{
	pr_info("enter aml_breathled_shutdown\n");
	led_control(ALWAYS_OFF);
}
...
static int aml_breathled_probe(struct platform_device *pdev)
{
	...
	pr_info("enter aml_breathled_probe\n");
	...
	pr_info("start read breathled dts\n");
	...
	pr_info("pwmb reg addr = %p,size=%d\n", pdata->reg_base,
	       pdata->size);
	/*set pin ctl */
	pin_ctl = devm_pinctrl_get_select(&pdev->dev, "aml_breathled_pin");
	if (IS_ERR(pin_ctl))
		pr_err("aml breathled pinmux set error!\n");

	/* read dev_name */
	ret_dts =
	    of_property_read_string(pdev->dev.of_node, "dev_name",
				    &pdata->name);
	if (ret_dts) {
		dev_err(&pdev->dev, "read %s  error\n", "dev_name");
		goto err;
	}
	pr_info("breathled pdata->name:%s\n", pdata->name);

	/* read pwm_prescale */
	value = -1;
	ret_dts =
	    of_property_read_u32(pdev->dev.of_node, "pwm_prescale", &value);
	if (ret_dts) {
		dev_err(&pdev->dev, "read %s  error\n", "pwm_prescale");
		goto err;
	}
	...
	pr_info("breathled pdata->pwm_prescale:%d\n", pwm_prescale);

	/* read pwm_entire_cycle */
	value = -1;
	ret_dts =
	    of_property_read_u32(pdev->dev.of_node, "pwm_entire_cycle", &value);
	if (ret_dts) {
		dev_err(&pdev->dev, "read %s  error\n", "pwm_entire_cycle");
		goto err;
	}
	...
	pr_info("breathled pdata->pwm_entire_cycle:0x%x\n",
	       pwm_entire_cycle);
	...
	if (ret) {
		pr_err("class_register breathled_class failed\n");
		goto err;
	}
	/*PWM init */
 ...
 err1:
	...
	pr_err("Canvas driver probe failed\n");
	return retval;
}
...
static int __exit aml_breathled_remove(struct platform_device *pdev)
{
	...
	pr_info("enter aml_breathled_remove\n");
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&early_suspend);
#endif
	devm_iounmap(&pdev->dev, pdata->reg_base);
	devm_release_mem_region(&pdev->dev, pdata->res.start, pdata->size);
...
static int __init aml_breathled_init(void)
{
	...
	pr_info("enter aml_breathled_init\n");
	ret = platform_driver_register(&aml_breathled_driver);

	if (ret != 0) {
		pr_err("failed to register breathled driver, error %d\n", ret);
		return -ENODEV;
	}
	return ret;
}
